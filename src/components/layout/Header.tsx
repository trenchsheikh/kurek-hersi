...
export function Header() {
  ...
  return (
    <header
      onClick={(e) => {
        // Only scroll to top if the user clicks the header itself, not the button
        if (e.target === e.currentTarget || (e.target as HTMLElement).tagName === 'DIV') {
          window.scrollTo({ top: 0, behavior: "smooth" });
        }
      }}
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 bg-black/40 backdrop-blur-md border-b border-white/5 cursor-pointer ${
        isScrolled ? "py-0 shadow-xl" : "py-0"
      }`}
    >
      <div className="w-full flex items-center relative min-h-[70px] md:min-h-[80px]">
